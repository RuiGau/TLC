...
(** Register the widget to the window.
	...
	@return			New window. *)
(*let window_register window widget =
	let (_, _, METHOD inst) = widget in
	snd (inst widget window REGISTER)*)
...
let rec label_inst text (widget: 'a widget) window msg =
	...
	| GET_SIZE ->
		...
		(SIZE (
			...
			th + (pad_top conf) + (pad_bottom conf)), window)
	| DRAW ->
		let conf = widget_conf widget in
...
let statusbar_make (win: 'a window) name msg =
	let window = label_make win name msg in
...
let hbox_inst names widget window msg =
	...
	| SET_BOX (x, y, w, h) ->
		let window = widget_change_box window widget (x, y, w, h) in
		let (win, x) =
			List.fold_right (fun name (win, x) ->
				match widget_call win name GET_SIZE with
				| (SIZE (cw, ch), win) ->
					let win = widget_set_box win name (x, y, cw, h) in
					(win, x + cw)
				| _ -> failwith "bug")
				names (window, x) in
			(NOTHING, win)
...
let rec console_inst rline lines widget window msg =
	...
	let add_line line =
		let (x, y, w, h) = widget_box widget in
		let conf = widget_conf widget in
		(*let ax = left x w conf in
		let ay = up y h conf in
		let aw = w - (pad_left conf) - (pad_right conf) in*)
		...
		(*let lines = 
			if lcnt < max
			then begin
				ignore (display_line conf ax line (ay - lh * (lcnt + 1)));
				line :: lines
			end
			else begin
				let image = Graphics.get_image
					ax
					ay
					aw
					((lcnt - 1) * lh) in
				Graphics.draw_image image
					(x + (pad_left conf))
					(y + (pad_bottom conf) + lh);
				Graphics.set_color (back_color conf);
				Graphics.fill_rect ax (ay - lh * max) aw lh;
				ignore (display_line conf ax line (ay - lh * max));
				line :: (remove lines)
			end in
	...
	| GET_SIZE ->
		...
		(SIZE (
			...
			h + (pad_top conf) + (pad_bottom conf)),
		window)
